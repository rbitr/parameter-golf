...
- [x] **Test-time training (TTT)** — TRIED: Full-model SGD TTT. SOTA defaults (lr=0.002, 3ep) caused catastrophic forgetting (+0.023 BPB). Conservative (lr=0.0005, 1ep) gives **-0.0012 BPB** (best delta). lr=0.001: -0.0010. Freeze 6 blocks: +0.0005 worse. Anchor alpha=0.0003: delta=-0.0007. **2 epochs at lr=0.0005: delta=-0.0007** (extra epoch causes forgetting). **3ep+2freeze (SOTA config): delta=+0.0003 WORSE**. **freeze=2, lr=0.001, 2ep: delta=+0.0019 WORSE** — even with frozen early layers, lr×epochs too high. SOTA's model handles aggressive TTT because of stronger base (BigramHash 1536d, 7185 steps). **All TTT configs exhausted. Best: 1ep/0freeze/lr=0.0005 → delta=-0.0012.**
