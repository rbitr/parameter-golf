...
def main() -> None:
    ...
    # EMA+SWA blend search: find optimal combination of EMA and SWA weights
    current_state = base_model.state_dict()
    swa_avg = None
    if swa_state is not None and swa_count > 0:
        swa_avg = {name: (t.float() / swa_count) for name, t in swa_state.items()}
        log0(f"swa:collected {swa_count} checkpoints, blending with EMA")
    else:
        log0("swa:no checkpoints collected, using pure EMA")
    best_blend_alpha = 1.0  # 1.0 = pure EMA, 0.0 = pure SWA
    best_blend_bpb = float("inf")
    blend_alphas = [1.0] if swa_avg is None else [1.0, 0.7, 0.5, 0.3]
    for alpha in blend_alphas:
        if alpha == 1.0:
            blended = {name: t.to(dtype=current_state[name].dtype) for name, t in ema_state.items()}
        else:
            blended = {}
            for name in ema_state:
                ema_f = ema_state[name].float()
                swa_f = swa_avg[name].to(device=ema_f.device).float()
                blend_val = alpha * ema_f + (1.0 - alpha) * swa_f
                blended[name] = blend_val.to(dtype=current_state[name].dtype)
        ...
        torch.cuda.synchronize()
        log0(
            f"BLEND alpha={alpha:.1f} val_loss:{diag_val_loss:.4f} val_bpb:{diag_val_bpb:.4f} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_diag):.0f}ms"
        )
        if diag_val_bpb < best_blend_bpb:
            best_blend_bpb = diag_val_bpb
            best_blend_alpha = alpha
    # Apply best blend
    if best_blend_alpha == 1.0:
        ...
        for name in ema_state:
            ema_f = ema_state[name].float()
            swa_f = swa_avg[name].to(device=ema_f.device).float()
            blend_val = best_blend_alpha * ema_f + (1.0 - best_blend_alpha) * swa_f
            avg_state[name] = blend_val.to(dtype=current_state[name].dtype)
    base_model.load_state_dict(avg_state, strict=True)
    log0(f"BEST_BLEND alpha={best_blend_alpha:.1f} val_bpb:{best_blend_bpb:.4f}")
