...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.vr_lambda = nn.Parameter(torch.full((1,), -4.0, dtype=torch.float32))
        self.rope_dims = 0
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        # Gated attention: per-head sigmoid gate (init=4.0 → sigmoid≈0.98, near identity)
        gate = torch.sigmoid(self.attn_gate.to(dtype=y.dtype))[None, None, :, None]
        y = y * gate
        # Value residual: add gated residual of original values (init=-4.0 → sigmoid≈0.02, near zero)
        vr_lam = torch.sigmoid(self.vr_lambda.to(dtype=y.dtype))
        group = self.num_heads // self.num_kv_heads
        v_expanded = v.unsqueeze(3).expand(bsz, seqlen, self.num_kv_heads, group, self.head_dim).reshape(bsz, seqlen, self.num_heads, self.head_dim)
        y = y + vr_lam * v_expanded
...
class SwiGLUMLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
        # Match parameter count: 3*H'*D = 2*H*D where H = mlp_mult*D
        # So H' = 2/3 * mlp_mult * D
...
class GPT(nn.Module):
    def __init__(
        ...
        # Cross-layer KV sharing: pairs of layers share K and V projections
        if kv_share_stride >= 2:
            for i in range(0, num_layers, kv_share_stride):
                for j in range(i + 1, min(i + kv_share_stride, num_layers)):
                    self.blocks[j].attn.c_k = self.blocks[i].attn.c_k
                    self.blocks[j].attn.c_v = self.blocks[i].attn.c_v
        if rope_dims > 0:
...
def eval_val_sliding_ttt(
    ...
    # Save original weights for anchor regularization
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        optimizer.step()
                        # Anchor regularization: pull weights back toward original
...
def quantize_int6_grouped(t: Tensor, clip_range: int = 31, group_size: int = _QUANT_GROUP_SIZE) -> tuple[Tensor, Tensor]:
    """Grouped int6 quantization: each group of `group_size` elements in a row gets its own scale."""
    t32 = t.float()
    if t32.ndim == 2:
        rows, cols = t32.shape
        if cols % group_size == 0 and cols > group_size:
            num_groups = cols // group_size
            # Reshape to (rows, num_groups, group_size)
            t_g = t32.reshape(rows, num_groups, group_size)
            # Per-group optimal MSE search over clip percentiles
            best_q = None
            best_s = None
            best_err = float('inf')
            for pct in [0.999, 0.9997, 0.9999, 0.99999, 1.0]:
                if pct < 1.0:
                    # Compute per-group clip values
                    g_clip = torch.quantile(t_g.abs(), pct, dim=2)  # (rows, num_groups)
                else:
                    g_clip = t_g.abs().amax(dim=2)  # (rows, num_groups)
                s = (g_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
                q = torch.clamp(
                    torch.round(t_g / s.float().unsqueeze(2)),
                    -clip_range, clip_range
                ).to(torch.int8)
                recon = q.float() * s.float().unsqueeze(2)
                err = (t_g - recon).pow(2).mean().item()
                if err < best_err:
                    best_q, best_s, best_err = q.reshape(rows, cols), s, err
            return best_q, best_s  # q: (rows, cols) int8, s: (rows, num_groups) fp16
        else:
            # Fall back to per-row quantization
            return quantize_int6_per_row(t, clip_range)
    # 1D or scalar: same as per-row
    amax = t32.abs().max().item()
    scale = torch.tensor(amax / clip_range if amax > 0 else 1.0, dtype=torch.float16)
    q = torch.clamp(torch.round(t32 / scale.float()), -clip_range, clip_range).to(torch.int8)
    return q, scale
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], hessians: dict[str, Tensor] | None = None):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            h_key = name
            if hessians is not None and h_key in hessians and t.ndim == 2:
                q, s = quantize_int6_gptq(t, hessians[h_key])
                gptq_count += 1
            else:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        kv_share_stride=args.kv_share_stride,
    ).to(device).bfloat16()
    ...
    if args.kv_share_stride >= 2:
        shared_pairs = [(i, min(i+args.kv_share_stride-1, args.num_layers-1)) for i in range(0, args.num_layers, args.kv_share_stride)]
        log0(f"kv_share:stride={args.kv_share_stride} pairs={shared_pairs}")
    ...
    log0(f"gptq:collected hessians for {len(hessians)} layers in {time.perf_counter()-t_hess:.1f}s")
    del ar_tokens
    torch.cuda.empty_cache()
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, hessians=hessians)
    del hessians
    # --- Magnitude-based pruning to fit under 16MB artifact limit ---
    MAX_ARTIFACT = 16_000_000
    code_bytes_est = len(code.encode("utf-8"))
    max_model_bytes = MAX_ARTIFACT - code_bytes_est
    def _compress_quant(qr, qm):
        buf = io.BytesIO()
        torch.save({"w": qr, "m": qm}, buf, _use_new_zipfile_serialization=False)
        raw = buf.getvalue()
        if _COMPRESSOR == "brotli":
            return brotli.compress(raw, quality=10), raw
    ...
    if len(quant_blob) > max_model_bytes:
        log0(f"prune:artifact {len(quant_blob)} > {max_model_bytes}, starting magnitude pruning...")
        # Collect all int6 quantized weight importances
        all_importances = []
        q_keys = []
        for name, info in quant_meta.items():
            if isinstance(info, dict) and info.get("type") == "int6":
                qk = name + ".q"
                sk = name + ".scale"
                if qk in quant_result and sk in quant_result:
                    q = quant_result[qk]
                    s = quant_result[sk]
                    # importance = |q| * scale (per-row scales)
                    sf = s.float()
                    if sf.ndim == 0:
                        imp = q.abs().float() * sf.item()
                    else:
                        imp = q.abs().float() * sf.view(q.shape[0], *([1]*(q.ndim-1)))
                    all_importances.append(imp.flatten())
                    q_keys.append(qk)
        all_imp = torch.cat(all_importances)
        nonzero_mask = all_imp > 0
        nonzero_imp = all_imp[nonzero_mask]
        log0(f"prune:total_weights={all_imp.numel()} nonzero={nonzero_imp.numel()}")
        # Binary search on percentile threshold
        lo, hi = 0.0, 20.0  # percentile range to search
        best_blob, best_raw, best_pct = quant_blob, quant_raw, 0.0
        for _iter in range(8):
            ...
            for k, v in quant_result.items():
                qr_pruned[k] = v.clone() if k.endswith(".q") else v
            for name, info in quant_meta.items():
                if isinstance(info, dict) and info.get("type") == "int6":
                    qk = name + ".q"
                    sk = name + ".scale"
                    if qk in qr_pruned and sk in quant_result:
                        q = qr_pruned[qk]
                        s = quant_result[sk].float()
                        if s.ndim == 0:
                            imp = q.abs().float() * s.item()
                        else:
                            imp = q.abs().float() * s.view(q.shape[0], *([1]*(q.ndim-1)))
                        q[imp < threshold] = 0
            blob, raw = _compress_quant(qr_pruned, quant_meta)
            pct_pruned = mid
            log0(f"prune:iter={_iter} pct={mid:.2f}% threshold={threshold:.6f} size={len(blob)} target={max_model_bytes}")
            if len(blob) <= max_model_bytes:
                best_blob, best_raw, best_pct = blob, raw, pct_pruned
                hi = mid  # try less pruning
            else:
                ...
                if isinstance(info, dict) and info.get("type") == "int6":
                    ...
                    if qk in quant_result and sk in quant_result:
                        ...
                        if s.ndim == 0:
                            ...
                            imp = q.abs().float() * s.view(q.shape[0], *([1]*(q.ndim-1)))
                        pruned_count = (imp < threshold).sum().item()
                        q[imp < threshold] = 0
                        log0(f"prune:applied {name} pruned={pruned_count}/{q.numel()} ({100*pruned_count/q.numel():.1f}%)")
            quant_blob, quant_raw = best_blob, best_raw
            log0(f"prune:DONE pct={best_pct:.2f}% final_size={len(quant_blob)} fits={len(quant_blob)<=max_model_bytes}")
        else:
            log0(f"prune:no pruning needed or 0% already fits, size={len(quant_blob)}")
    else:
        log0(f"prune:artifact {len(quant_blob)} <= {max_model_bytes}, no pruning needed")
    if master_process:
        ...
        lzma_blob = _lzma.compress(quant_raw, preset=9)
        log0(f"COMPARE lzma_preset9: {len(lzma_blob)} bytes (total {len(lzma_blob) + code_bytes} bytes)")
    if distributed:
        dist.barrier()
