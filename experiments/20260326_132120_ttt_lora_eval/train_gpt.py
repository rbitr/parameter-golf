...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None, q_delta_fn=None, v_delta_fn=None) -> Tensor:
        ...
        qd = q_delta_fn(norm_in) if q_delta_fn is not None else None
        vd = v_delta_fn(norm_in) if v_delta_fn is not None else None
        attn_out = self.attn(norm_in, v_embed=v_embed, q_delta=qd, v_delta=vd)
