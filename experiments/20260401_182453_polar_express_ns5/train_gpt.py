...
def zeropower_via_newtonschulz5(G: Tensor, steps: int = 5, eps: float = 1e-7) -> Tensor:
    # Polar Express (Amsel et al., ICLR 2026): per-iteration optimal quintic polynomials
    # Converges cubically vs linearly for fixed NS coefficients
...
def quantize_int6_gptq(weight, hessian, clip_range=31, block_size=128):
    ...
    try:
        ...
        H[torch.arange(H.size(0), device=H.device), torch.arange(H.size(0), device=H.device)] += extra_damp
        Hinv = torch.linalg.cholesky(H)
